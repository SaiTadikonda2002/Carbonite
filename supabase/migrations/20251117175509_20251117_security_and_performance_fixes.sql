/*
  ...
  1. Add missing index on foreign key (user_actions.action_template_id)
  2. Optimize RLS policies to use (select auth.uid()) instead of auth.uid()
  3. Fix function search_path to be immutable
  4. Note: Unused indexes are kept as they support query patterns
*/

-- 1. Add missing index on foreign key
CREATE INDEX IF NOT EXISTS idx_user_actions_action_template_id ON user_actions(action_template_id);
...
DROP POLICY "Users can insert own actions" ON user_actions;
CREATE POLICY "Users can insert own actions" ON user_actions
  FOR INSERT WITH CHECK ((select auth.uid()) = user_id);

DROP POLICY "Users can update own actions" ON user_actions;
